...
// codes
var calWidth=200, calHeight=230, calOffsetX=-200, calOffsetY=16;
var calWin=null;
var winX=0, winY=0;
var cal="cal";
var cals=new Array();
...
function goToMonth(month){
	cM = month;
	var nd = new Date(cY,month,1);
	changeCal(nd);

}


function createMonthYear(d){
	s="<tr align='center' bgcolor='"+titleColor+"'><td colspan='7'>\n<table><tr><td><select onchange='javascript:if(window.opener && !window.opener.closed && window.opener.goToMonth) window.opener.goToMonth(this.options[this.selectedIndex].value)'>";
	for(i=0; i <yxMonths.length; i++){
		selected = "";
		if(i==d.getMonth())
			selected = " selected";
		s+="<option value='"+i+"' "+selected+">"+yxMonths[i]+"</option>";
	}
	s+="</select><td><select onchange='window.opener.goToYear(this.options[this.selectedIndex].value)'>";
	curYear = parseInt((new Date()).getFullYear());
	for(i=curYear - 20; i < curYear + 20; i++){
		selected = "";
		if(i==d.getFullYear())
			selected = " selected";
		s+="<option value='"+i+"'"+selected+">"+i+"</option>";
	}
	s+="</select></td></tr></table></td></tr>";
	return s;
}

function calTitle(d) {
  var yy=d.getFullYear(), mm=yxMonths[d.getMonth()];
  var s = createMonthYear(d);
  if (titleMode == 2) {
...
function showCal(name) {
  ...
  if (currentCal != null && currentCal.form != null && currentCal.form[currentCal.field]) {
    ...
    if (calWin != null && !calWin.closed) {
	  ...
	  var winl = (screen.width-calWidth)/2;
	  var wint = (screen.height-calHeight)/2;
	  if (winl < 0) winl = 0;
	  if (wint < 0) wint = 0;
    ...
    if (!hasCal) {
      if (isIE || isOpera6) {
        calWin=window.open("","cal","toolbar=0,width="+calWidth+",height="+calHeight+",left="+(winl)+",top="+(wint));
      }
      else {
        calWin=window.open("","cal","toolbar=0,width="+calWidth+",height="+calHeight+",screenx="+(winl)+",screeny="+(wint));
      }
    }
...
function showCal1(name,date) {
  var lastCal=currentCal;
  var d=new Date(), hasCal=false;
  
  var ya = d.getYear().toString();
  d.setDate(date.substring(0,2));
  d.setMonth(getMonthNumber(date.substring(3,6)));
  d.setYear(ya.substring(0,2).concat(date.substring(7,9)));
  ...
  if (currentCal != null && currentCal.form != null && currentCal.form[currentCal.field]) {
    ...
    if (currentCal.form[currentCal.field].value!="" && calRE.test(currentCal.form[currentCal.field].value)) {
      ...
      cY=ya.substring(0,2).concat(date.substring(7,9));//cd[0];
      cM=getMonthNumber(date.substring(3,6));//cd[1];
      dd=date.substring(0,2);//cd[2];
    }
    else {
      cY=d.getFullYear();
      cM=d.getMonth();
...
function pickDate(d) {
  ...
  var tempYear=cY+"";
  var date=calFormat;
  var tempDate="dd-mmm-yy";
  tempDate=tempDate.replace(/dd/i,get2Digits(d));
  tempDate=tempDate.replace(/mmm/i,getMonthName(cM));
  tempDate=tempDate.replace(/yy/i,tempYear.substring(2,4));
//  alert("TempDate:"+tempDate);
//  date = date.replace(/yyyy/i, tempYear.substring(2,4));
//  date = date.replace(/mm/i, getMonthName(cM+1));
...
//  alert(date);
  currentCal.form[currentCal.field].value=tempDate;
